...
function onJoin( ply )
	...
	if row then
		if utime_welcome:GetBool() then
			ULib.tsay( ply, "[SERVEUR]Salut, " .. ply:Nick() .. ", votre derniere connection sur le serveur est le " .. os.date( "%c", row.lastvisit ) )
		end
		sql.Query( "UPDATE utime SET lastvisit = " .. os.time() .. " WHERE player = " .. uid .. ";" )
		time = row.totaltime
	else
		if utime_welcome:GetBool() then
			ULib.tsay( ply, "[SERVEUR]Bienvenue sur le serveur " .. ply:Nick() .. "!" )
		end
